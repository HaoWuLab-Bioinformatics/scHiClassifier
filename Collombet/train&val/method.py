...
class montage_model(nn.Module):
    def __init__(self, model_para):
        ...
        linear_size_SICP_init = num_hiddens
        linear_size_BCP_init = num_hiddens
        linear_size_SSICP_init = num_hiddens
        linear_size_SBCP_init = num_hiddens
        self.Con_layer_SICP, self.Con_layer_BCP, self.Con_layer_SSICP, self.Con_layer_SBCP = Con_layer
        linear_size_SICP = com_linearsize(linear_size_SICP_init, self.Con_layer_SICP, kernel_size)
        ...
        if self.Con_layer_SICP != 0:
            ...
            self.bn1_SICP = nn.BatchNorm1d(num_features=out_channels)
            self.rule1_SICP = nn.ReLU()
            self.pool_SICP = nn.MaxPool1d(kernel_size=2)
            self.dropout_SICP = nn.Dropout(dp)
            self.Con_SICP = nn.Sequential()
            for i in range(self.Con_layer_SICP - 1):
                layer_id = str(i + 2)
                self.Con_SICP.add_module("conv%s" % layer_id,
                                        nn.Conv1d(in_channels=out_channels, out_channels=out_channels,
                                                  kernel_size=kernel_size, stride=1, padding=1))
                self.Con_SICP.add_module("bach%s" % layer_id, nn.BatchNorm1d(num_features=out_channels))
                self.Con_SICP.add_module("relu%s" % layer_id, nn.ReLU())
                self.Con_SICP.add_module("maxp%s" % layer_id, nn.MaxPool1d(kernel_size=2))
                self.Con_SICP.add_module("drop%s" % layer_id, nn.Dropout(dp))
        if self.Con_layer_BCP != 0:
            ...
            self.bn1_BCP = nn.BatchNorm1d(num_features=out_channels)
            ...
            for i in range(self.Con_layer_BCP - 1):
                ...
                self.Con_BCP.add_module("conv%s" % layer_id,
                                        nn.Conv1d(in_channels=out_channels, out_channels=out_channels,
                                                  kernel_size=kernel_size, stride=1, padding=1))
                self.Con_BCP.add_module("bach%s" % layer_id, nn.BatchNorm1d(num_features=out_channels))
        ...
        if linear_layer == 1:
            self.fc = nn.Linear(in_features=out_channels * (linear_size_SICP+linear_size_BCP+linear_size_SSICP+linear_size_SBCP), out_features=num_types)
        else:
            self.fc1 = nn.Linear(in_features=out_channels * (linear_size_SICP+linear_size_BCP+linear_size_SSICP+linear_size_SBCP), out_features=out_feature)
            self.relu2 = nn.ReLU()
    ...
    def forward(self, X):
        ...
        x4 = X[:, 3:4, :]
        if self.Con_layer_SICP != 0:
            x1 = self.rule1_SICP(self.bn1_SICP(self.conv1_SICP(x1)))
            x1 = self.pool_SICP(x1)
            x1 = self.dropout_SICP(x1)
            x1 = self.Con_SICP(x1)
            x1 = x1.view(-1, self.out_channels * self.linear_size_SICP)
        if self.Con_layer_BCP != 0:
            x2 = self.rule1_BCP(self.bn1_BCP(self.conv1_BCP(x2)))
            x2 = self.pool_BCP(x2)
            x2 = self.dropout_BCP(x2)
            x2 = self.Con_BCP(x2)
            x2 = x2.view(-1, self.out_channels * self.linear_size_BCP)
        if self.Con_layer_SSICP != 0:
            x3 = self.rule1_SSICP(self.bn1_SSICP(self.conv1_SSICP(x3)))
            x3 = self.pool_SSICP(x3)
            x3 = self.dropout_SSICP(x3)
            x3 = self.Con_SSICP(x3)
            x3 = x3.view(-1, self.out_channels * self.linear_size_SSICP)
        if self.Con_layer_SBCP != 0:
            x4 = self.rule1_SBCP(self.bn1_SBCP(self.conv1_SBCP(x4)))
            x4 = self.pool_SBCP(x4)
            x4 = self.dropout_SBCP(x4)
            x4 = self.Con_SBCP(x4)
            x4 = x4.view(-1, self.out_channels * self.linear_size_SBCP)

        if self.Con_layer_SICP != 0 and self.Con_layer_BCP != 0 and self.Con_layer_SSICP != 0 and self.Con_layer_SBCP != 0:
            x = torch.cat((x1, x2, x3, x4), 1)
        elif self.Con_layer_SICP == 0 and self.Con_layer_BCP != 0 and self.Con_layer_SSICP != 0 and self.Con_layer_SBCP != 0:
...
def CNN_val(epoch, model, val_loader, loss_fn, test_size, device):
    i = 0
    for (tensors, labels) in val_loader:
        ...
        val_result_matrix0 = outputs
        val_result_matrix = val_result_matrix0.cpu().detach().numpy()
        _, prediction = torch.max(outputs.data, 1)
        ...
        print('Val Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
            epoch, i * len(tensors), len(val_loader.dataset),
                   100. * i / len(val_loader), val_loss.cpu().data))
        i = i+1
    return label_pred, label, val_loss, model, val_accuracy, val_result_matrix


def CNN_1D_montage_val(f1_Data, f2_Data, f3_Data, f4_Data, tr_x, tr_y, val_x, val_y, lr, fold, model_para, alpha, gamma, batch_size, testseed):
    train_X_f1, train_size_f1 = load_Feature_data(f1_Data, tr_x)
    val_X_f1, val_size_f1 = load_Feature_data(f1_Data, val_x)
    train_X_f2, train_size_f2 = load_Feature_data(f2_Data, tr_x)
    val_X_f2, val_size_f2 = load_Feature_data(f2_Data, val_x)
    train_X_f3, train_size_f3 = load_Feature_data(f3_Data, tr_x)
    val_X_f3, val_size_f3 = load_Feature_data(f3_Data, val_x)
    train_X_f4, train_size_f4 = load_Feature_data(f4_Data, tr_x)
    val_X_f4, val_size_f4 = load_Feature_data(f4_Data, val_x)

    train_X_f1 = torch.unsqueeze(torch.from_numpy(np.array(train_X_f1).astype(float)), dim=1)
    val_X_f1 = torch.unsqueeze(torch.from_numpy(np.array(val_X_f1).astype(float)), dim=1)
    train_X_f2 = torch.unsqueeze(torch.from_numpy(np.array(train_X_f2).astype(float)), dim=1)
    val_X_f2 = torch.unsqueeze(torch.from_numpy(np.array(val_X_f2).astype(float)), dim=1)
    train_X_f3 = torch.unsqueeze(torch.from_numpy(np.array(train_X_f3).astype(float)), dim=1)
    val_X_f3 = torch.unsqueeze(torch.from_numpy(np.array(val_X_f3).astype(float)), dim=1)
    train_X_f4 = torch.unsqueeze(torch.from_numpy(np.array(train_X_f4).astype(float)), dim=1)
    val_X_f4 = torch.unsqueeze(torch.from_numpy(np.array(val_X_f4).astype(float)), dim=1)

    train_X = torch.cat((train_X_f1, train_X_f2, train_X_f3, train_X_f4), 1)  # 在1维进行拼接
    val_X = torch.cat((val_X_f1, val_X_f2, val_X_f3, val_X_f4), 1)  # 在1维进行拼接

    train_dataset = TensorDataset(train_X, torch.from_numpy(np.array(tr_y).astype(int)))
    val_dataset = TensorDataset(val_X, torch.from_numpy(np.array(val_y).astype(int)))
    val_size = val_size_f1  # 103
    train_loader, val_loader = load_loader(train_dataset, val_dataset, val_size, batch_size)
    ...
    patience0 = 40
    early_stopping = EarlyStopping(patience=patience0, verbose=True)

    optimizer = Adam(model.parameters(), lr=lr)
    loss_fn = MultiClassFocalLossWithAlpha(alpha=alpha, gamma=gamma)
    path = "./model_sa+Conv1d_earlystopping/zuixin_max_epoch200"
    for epoch in range(num_epochs):
        model.train()
        model, optimizer = CNN_train(epoch, model, optimizer, train_loader, loss_fn, device)
        model.eval()
        val_label, label, val_loss, model, val_accuracy, val_result_matrix = CNN_val(epoch, model, val_loader,
                                                                  loss_fn, val_size_f1, device)
        real_label = label
        val_macro_F1 = f1_score(real_label, val_label, average='macro')
        #  def __call__(self, val_loss, model_testupdate5, path, inter_fold):
        # 看一下这个early_stopping
        early_stopping(val_loss, model, path, fold, testseed, patience0)
    ...
    return val_accuracy, val_label, label, val_result_matrix
