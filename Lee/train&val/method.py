...
from focal_loss_GPU1 import MultiClassFocalLossWithAlpha
...
class montage_model(nn.Module):
    ...
    def forward(self, X):
        X = self.encoder(X)
        # X的shape为[batch_size,num_feature,3060]
        # X1的shape为[batch_size,1,3060],取了第一个特征
        x1 = X[:, 0:1, :]
        x2 = X[:, 1:2, :]
        x3 = X[:, 2:3, :]
        x4 = X[:, 3:4, :]
...
def CNN_1D_montage_val(f1_Data, f2_Data, f3_Data, f4_Data, tr_x, tr_y, val_x, val_y, lr, fold, model_para, alpha, gamma, batch_size, testseed):
    ...
    # print(train_X_f1.shape)  # torch.Size([训练集中训练子集的细胞个数, 1, 3053])
    # print(val_X_f1.shape)    # torch.Size([训练集中验证子集的细胞个数, 1, 3053])
    #  train_X_f1的Size([3292, 1, 3053])，由于3053在进多头自注意力的时候不能被20整除，所以需要补7个0
    zeros_train_f1 = torch.zeros(train_size_f1, 1, 7)
    zeros_val_f1 = torch.zeros(val_size_f1, 1, 7)
    zeros_train_f2 = torch.zeros(train_size_f2, 1, 7)
    zeros_val_f2 = torch.zeros(val_size_f2, 1, 7)
    zeros_train_f3 = torch.zeros(train_size_f3, 1, 7)
    zeros_val_f3 = torch.zeros(val_size_f3, 1, 7)
    zeros_train_f4 = torch.zeros(train_size_f4, 1, 7)
    zeros_val_f4 = torch.zeros(val_size_f4, 1, 7)
    train_X_f1 = torch.cat((train_X_f1, zeros_train_f1), 2)
    val_X_f1 = torch.cat((val_X_f1, zeros_val_f1), 2)
    train_X_f2 = torch.cat((train_X_f2, zeros_train_f2), 2)
    val_X_f2 = torch.cat((val_X_f2, zeros_val_f2), 2)
    train_X_f3 = torch.cat((train_X_f3, zeros_train_f3), 2)
    val_X_f3 = torch.cat((val_X_f3, zeros_val_f3), 2)
    train_X_f4 = torch.cat((train_X_f4, zeros_train_f4), 2)
    val_X_f4 = torch.cat((val_X_f4, zeros_val_f4), 2)
    ...
    # print(train_X.shape)  # torch.Size([训练集中训练子集的细胞个数, 4, 3060])
    # print(val_X.shape)   # torch.Size([训练集中验证子集的细胞个数,  4, 3060])
    ...
    loss_fn = MultiClassFocalLossWithAlpha(alpha=alpha, gamma=gamma)
    path = "./model_sa+Conv1d_earlystopping/zuixin_max_epoch500"
