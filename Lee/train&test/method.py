...
class montage_model(nn.Module):
    ...
    def forward(self, X):
        X = self.encoder(X)
        # X's shape is [batch_size,num_feature,3060]
        # X1's shape is [batch_size,1,3060], took the first feature
        x1 = X[:, 0:1, :]
        x2 = X[:, 1:2, :]
        x3 = X[:, 2:3, :]
        x4 = X[:, 3:4, :]
...
def CNN_1D_montage(f1_Data, f2_Data, f3_Data, f4_Data, x_train, y_train, X_test, y_test, lr, model_para, alpha, gamma, batch_size):
    ...
    # print(train_X_f1.shape) # torch.Size([number of cells in training set, 1, 3053])
    # print(test_X_f1.shape) # torch.Size([number of cells in testing set, 1, 3053])
    # train_X_f1's Size([number of cells in training set, 1, 3053]), which needs to be supplemented with 7 zeros since 3053 is not divisible by 20 when going into the TransformerEncoder
    ...
    # print(train_X.shape)  # torch.Size([number of cells in training set, 4, 3060])
    # print(test_X.shape)   # torch.Size([number of cells in testing set, 4, 3060])
