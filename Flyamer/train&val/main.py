...
from sklearn.metrics import f1_score, precision_score, balanced_accuracy_score, recall_score
                                                    ...
                                                    for gamma in [1,2,3,4,5]:
                                                        for batch_size in [8, 16, 32, 64]:
                                                            ...
                                                            for testseed in testseed_list:
                                                                ...
                                                                print(testseed)
                                                                Folds = StratifiedKFold(n_splits=5, shuffle=True, random_state=2024).split(x_train, y_train)
                                                                sum_val_accuracy = 0
                                                                sum_val_micro_F1 = 0
                                                                sum_val_macro_F1 = 0
                                                                ...
                                                                for fold, (tr_idx, val_idx) in enumerate(Folds):
                                                                    ...
                                                                    val_accuracy, val_label, label, val_result_matrix = CNN_1D_montage_val(f1_Data, f2_Data, f3_Data, f4_Data, tr_x, tr_y, val_x, val_y, lr, fold, model_para, alpha, gamma, batch_size, testseed)
                                                                    real_label = label
                                                                    val_micro_F1 = f1_score(real_label, val_label, average='micro')
                                                                    val_macro_F1 = f1_score(real_label, val_label, average='macro')
                                                                    val_micro_Precision = precision_score(real_label, val_label, average='micro')
                                                                    val_macro_Precision = precision_score(real_label, val_label, average='macro')
                                                                    val_micro_Recall = recall_score(real_label, val_label, average='micro')
                                                                    val_macro_Recall = recall_score(real_label, val_label, average='macro')
                                                                    val_bacc = balanced_accuracy_score(real_label, val_label)
