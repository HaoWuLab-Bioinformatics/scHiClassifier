...
5. The folder "**train&val**" contains the code details for the schiClassifier framework to be trained on the training subsets and validated on the validation subsets.
  
7. The subfolder "**train&val_result**" is designated for storing the evaluation results of the model performance on the validation subsets, after being trained on the training subsets. The "**focal_loss_GPU0.py**", "**focal_loss_GPU1.py**", "**focal_loss_GPU2.py**" and "**focal_loss_GPU3.py**" files refer to the code that calls different GPUs to compute the "**focal loss**" loss function, respectively. The file "**main.py**" contains the main body of code for the schiClassifier framework to be trained on the training subsets and validated on the validation subsets, while the file "**method.py**" contains the declarations of certain functions and classes invoked by the "**main.py**".



10. The folder "**train&val**" contains the  这个文件夹中包含了scHiClassifier框架在训练子集上训练并在验证子集上进行验证的代码细节。其中"**model_sa+Conv1d_earlystopping**"文件夹用于存放模型在验证子集上早停时对应的所有权重参数所组成的字典，"**train&val_result**" 文件夹用于存储在训练子集上训练好的模型在验证子集上的分类性能评估结果，"**focal_loss_GPU0.py**","**focal_loss_GPU1.py**","**focal_loss_GPU2.py**"和"**focal_loss_GPU3.py**"分别指的是调用不同GPU来进行"**focal loss**" 损失函数计算的代码，"**main.py**"文件是scHiClassifier框架在训练集上训练并在测试集上测试的代码主体，"**method.py**"文件中声明了一些"**main.py**"文件中的代码需要调用的函数和类，"**pytorch_tools.py**"文件中声明了一些"**main.py**"文件中的代码需要调用的包括"**EarlyStopping**"在内的一些工具类。
11. The file "**combo_hg19.genomesize**" is the code of the network architecture.  /is the text file used to build Docker container images.  
